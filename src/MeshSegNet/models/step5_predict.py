...
if __name__ == '__main__':
    ...
    model_path = os.environ.get('MESHSEGNET_MODEL_PATH', './models')
    model_name = os.environ.get('MESHSEGNET_MODEL_NAME', 'MeshSegNet_Max_15_classes_72samples_lr1e-2_best.tar')

    mesh_path = os.environ.get('MESHSEGNET_MESH_PATH', './')  # need to define
    samples_env = os.environ.get('MESHSEGNET_SAMPLES', 'Example.stl')
    sample_filenames = [s.strip() for s in samples_env.split(',') if s.strip()] # need to define
    ...
    with torch.no_grad():
        for i_sample in sample_filenames:
            ...
            ids = np.asarray(mesh_d.cells, dtype=np.int32)
