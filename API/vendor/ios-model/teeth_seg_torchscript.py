...
if __name__ == "__main__":
    ...
    if jaw_type == "max":
        script_path = _resolve_model_path("max_teeth_seg_model_script.pt")
    elif jaw_type == "man":
        script_path = _resolve_model_path("man_teeth_seg_model_script.pt")
    else:
        raise ValueError(f"Unsupported jaw_type {jaw_type}")
    torch.cuda.set_device(device)
    model = torch.jit.load(str(script_path))
    model.to(device)
    model.eval()
